	...
	public static class SpriteNineSlicerContextMenu {
		...
		private static void ProcessSprites(bool autodetectBorders, bool trimAlpha, bool trimCenter) {
			...
			foreach (var sprite in sprites) {
				...
				var spriteInfo = new SpriteInfo(imagePath, sprite.texture.width, sprite.texture.height,
					sprite.border.ToInt4(), sprite.texture.filterMode);
		...
		public static List<Sprite> GetSelectedSprites() {
			...
			foreach (var selectedObject in selectedObjects) {
				...
				if (AssetDatabase.IsValidFolder(assetPath)) {
					var spritesInsideFolder = AssetDatabase.FindAssets("t:Sprite",
						new[] {assetPath});
					foreach (var sprite in spritesInsideFolder) {
						result.AddRange(AssetDatabase
							.LoadAllAssetsAtPath(AssetDatabase.GUIDToAssetPath(sprite)).OfType<Sprite>());
					}
				} else {
					result.AddRange(AssetDatabase
						.LoadAllAssetsAtPath(assetPath).OfType<Sprite>());  
				}
			}
		...
		private static bool CheckSelection() => Selection.objects != null && Selection.objects.Length > 0;
	}
}