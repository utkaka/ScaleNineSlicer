...
namespace Utkaka.ScaleNineSlicer.Editor.SpriteSlicing.Window
{
    public class TiledImageElement : VisualElement
    {
        ...
        private void DrawMeshes(MeshGenerationContext context)
        {
            ...
            {
                position = new Vector3(0.0f, rect.height),
                uv = new Vector2(0, tilingY),
                tint = tint,
            };
            _vertices[2] = new Vertex
            {
                position = new Vector3(rect.width, rect.height),
                uv = new Vector2(tilingX, tilingY),
                tint = tint,
            };
            _vertices[3] = new Vertex
            {
                position = new Vector3(rect.width, 0.0f),
                uv = new Vector2(tilingX, 0),
                tint = tint,
            };
            ...
            var trackMeshWriteData = (MeshWriteData)
                methodInfo?.Invoke(context, new object[] { 4, 6, texture, null, 2 });
